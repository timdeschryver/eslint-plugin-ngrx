...
| [ngrx/prefer-action-creator-in-of-type](https://github.com/timdeschryver/eslint-plugin-ngrx/tree/main/docs/rules/prefer-action-creator-in-of-type.md)                   | Using an `action creator` in `ofType` is preferred over `string`.                                                                 | suggestion  | warn (Best Practices)   | No      | No              | No           | No                        |
| [ngrx/prefer-concat-latest-from](https://github.com/timdeschryver/eslint-plugin-ngrx/tree/main/docs/rules/prefer-concat-latest-from.md)                                 | Use `concatLatestFrom` instead of `withLatestFrom` to prevent the selector from firing until the correct `Action` is dispatched.  | suggestion  | warn (Possible Errors)  | Yes     | No              | Yes          | No                        |
